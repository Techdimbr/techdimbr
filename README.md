<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0d1117&height=300&section=header&text=Deivis%20Irio%20Mayer&fontSize=80&animation=fadeIn&fontAlignY=38&desc=Product%20Security%20Engineer%20%7C%20Linux%20Specialist%20%7C%20Techdim%20Architect&descAlignY=55&descAlign=62" width="100%" alt="Techdim Banner" />
</div>

---

### 👨‍💻 Executive Profile
**Lead Systems Architect & Product Security Engineer** at [**Techdim R&D**](https://github.com/Techdimbr).

Specialist in **Linux Hardening**, **Security Automation**, and **Cognitive Architectures**. I combine tactical hardware expertise (1000+ interventions) with strategic **DevSecOps**, developing autonomous agents (Guardians) powered by **Google Gemini Pro**.

> *"My objective is to optimize the interface between Human Intent and Artificial Intelligence through secure, scalable Linux infrastructure."*

---

### 🛠️ Technical Arsenal

| **Core Infrastructure** | **Security Engineering** | **Cognitive Ops (AI)** |
|:---:|:---:|:---:|
| ![Linux](https://img.shields.io/badge/Debian%20Linux-Expert-A81D33?style=for-the-badge&logo=debian&logoColor=white) | ![Security](https://img.shields.io/badge/OS%20Hardening-ISO%2027001-blue?style=for-the-badge&logo=shield) | ![Kether](https://img.shields.io/badge/Arch-Kether__1%20Oracle-8E75B2?style=for-the-badge&logo=google-bard&logoColor=white) |
| ![Network](https://img.shields.io/badge/Network-TCP%2FIP%20%2F%20VPN-gray?style=for-the-badge&logo=cisco) | ![ZeroTrust](https://img.shields.io/badge/Architecture-Zero%20Trust-red?style=for-the-badge) | ![Gemini](https://img.shields.io/badge/Google%20AI-Gemini%201.5%20Pro-4285F4?style=for-the-badge&logo=google&logoColor=white) |

---

### 🚀 Engineering Portfolio (Proof of Work)

#### 🛡️ Flagship: Sentinel AI (The Guardian)
> **Autonomous Log Analysis Agent.**
> A pure Bash/Python implementation that ingests system logs (`/var/log/auth.log`) and utilizes **LLM Inference** to detect zero-day anomalies.
> * **Status:** Production Ready.
> * **Core:** `Bash` + `Gemini API` + `Systemd`.

[![Sentinel AI](https://img.shields.io/badge/View_Code-Sentinel_AI_Guardian-success?style=for-the-badge&logo=github)](https://github.com/Techdimbr/sentinel-ai-log-analyzer)

#### 🔮 R&D Roadmap: Project KETHER_1
Internal development track at Techdim Labs focused on High-Level Cognitive Automation.

| Component | Codename | Status | Description |
|:---|:---:|:---:|:---|
| **Neural Firewall** | `Oraculo_Net` | 🚧 Alpha | Adaptive nftables ruleset updated by threat intelligence feeds. |
| **System Optimizer** | `Kether_Core` | 🏗️ Design | Automated kernel tuning based on workload prediction. |

---

### 📈 Trajectory
<img src="http://ghchart.rshah.org/Techdimbr" alt="Techdimbr Github chart" />

* **2024-Present:** **Techdim Architect.** Developing the *Guardian* ecosystem for automated security.
* **2019-Present:** **Infra Analyst @ ReparaFácil.** Ensuring 99.9% uptime for critical banking/agribusiness clients.

---

<div align="center">
  <a href="https://linkedin.com/in/deivismayer">
    <img src="https://img.shields.io/badge/Connect_on-LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:techdimbrasil@gmail.com">
    <img src="https://img.shields.io/badge/Contact-Email_Me-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</div>
